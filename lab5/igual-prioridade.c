...
int wanna_read;	// quantos querem ler
...
typedef enum {R /*leitura*/, W /*escrita*/} turn_t;
turn_t ultimavez;	// de quem foi a ultima vez
...
void startRead() {
	LOCK(mtx);
	wanna_read++;
	while ( (ultimavez = R && wanna_write > 0) || writting > 0)
...
void startWrite() {
	LOCK(mtx);
	wanna_write++;
	while ( (ultimavez == W && wanna_read > 0) || writting > 0 || reading > 0 )
